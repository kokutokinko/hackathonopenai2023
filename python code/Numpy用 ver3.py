...
def reference_summarize(data):
   ...
   max_tokens=5000,  #トークン数を指定する
   messages=[
         #プロンプトの指定
         {"role": "system","content": "Please summarize the following text about to about 300 word.The text is about Numpy reference. Don't add extra information and use only character codes which included to Shift-JIS."
            },
         #要約したい文章を入れる
...
def add_to_csv(upper, section_data, item):
   ...
   print("")
   with open(file_name, 'a',encoding="shift_jis", errors='ignore', newline='') as csvfile:
...
base_url = "https://numpy.org/doc/stable/reference/generated/"
#書き出されるファイルの名前を選択
file_name = f"numpytest.csv"
...
#上位階層のリスト
upper = ["Numpy","Universal functions"]
...
for item in items:
   ...
   if(len(section_text) < 10000):#直接書き込み
